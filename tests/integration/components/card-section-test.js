...
module('Integration | Component | card-section', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(
      hbs`<CardSection @title={{this.title}} @subtitle={{this.subtitle}} @steps={{this.steps}}/>`
    );

    assert.equal(find('[data-test-title]').textContent.trim(), this.title);
    assert.equal(
      find('[data-test-subtitle]').textContent.trim(),
      this.subtitle
    );
  ...
  test("it doesn't render the subtitle", async function (assert) {
    ...
    await render(
      hbs`<CardSection @title={{this.title}} @steps={{this.steps}}/>`
    );
