...
module('Integration | Helper | character-background', function (hooks) {
  ...
  test('returns background style tag for a url', async function (assert) {
    await render(
      hbs`<div style={{character-background '/foo/bar.png'}}></div>`
    );

    assert.equal(
      find('div').getAttribute('style'),
  ...
  test('returns background for mulitple images', async function (assert) {
    ...
    await render(
      hbs`<div style={{character-background this.inputValue}}></div>`
    );

    assert.equal(
      find('div').getAttribute('style'),
      'background-image: url(/foo/bar.png),url(/foo/baz.png)'
    );
  });
});
